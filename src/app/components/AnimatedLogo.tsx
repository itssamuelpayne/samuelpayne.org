...
import { COLOR_START_OFFSET } from '../hooks/useAnimatedColor';

interface AnimatedLogoProps {
  size?: number;
}

// One full loop of gradient keyframes. The sequence is cyclic — every step
// flows smoothly into the next, including the wrap from last back to first —
// so we can begin anywhere and still loop seamlessly.
const GRADIENTS = [
...
// Rotate so the loop starts at `offset`, then re-append the start frame to
// close the cycle seamlessly.
function loopFrom(offset: number): string[] {
  const n = GRADIENTS.length;
  const o = ((offset % n) + n) % n;
  const seq = [...GRADIENTS.slice(o), ...GRADIENTS.slice(0, o)];
  return [...seq, seq[0]];
}

export function AnimatedLogo({ size = 240 }: AnimatedLogoProps) {
  const frames = loopFrom(COLOR_START_OFFSET);
